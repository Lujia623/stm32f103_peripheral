 ...
 */
 #define SYS_LIGHTWEIGHT_PROT    1     //* SYS_LIGHTWEIGHT_PROT==1:如果你想要确定的跨任务保护 *在缓冲区分配，回收和内存的关键区域 *分配和回收。
 
 /**
 * NO_SYS==1: Provides VERY minimal functionality. Otherwise,
 * use lwIP facilities.
 */
 #define NO_SYS                  0     //NO_SYS表示无操作系统模拟层，这个宏非常重要， 因为无操作系统与有操作系统的移植和编写是完全不一样的， 
 ...
 */
 ...
 #define MEM_SIZE                (10 * 1024)        //堆内存的大小。如果应用程序将发送很多需要复制的数据应该设置得大一点。     
 ...
 /* MEMP_NUM_TCP_PCB: the number of simulatenously active TCP
     connections. */
 #define MEMP_NUM_TCP_PCB        6   //TCP协议控制块的数量。一个每个活跃的tcp“连接”。
 ...
 #define TCP_SND_QUEUELEN        (8* TCP_SND_BUF/TCP_MSS)  //TCP发送端缓冲区空间(pbufs)。这一定是至少最多为(2 * TCP_SND_BUF/TCP_MSS)。
 
 /* TCP receive window. */
 #define TCP_WND                 (4*TCP_MSS)   //TCP接收窗口大小。      
 ...
 */
 #define LWIP_NETCONN                    1            //开启/关闭netconn功能
 ...
 */
 #define LWIP_SOCKET                     1            //开启/关闭socket功能
